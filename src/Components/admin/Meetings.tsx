    ...
    <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300">
      ...
      <div className="p-4 space-y-4">
        {meetings.map((meeting, index) => (
          <div
            key={index}
            className="rounded-lg p-4   hover:border-red-300 duration-200 bg-white border  border-gray-200"
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-xl ${meeting.badgeColor} font-medium`}
              >
                {meeting.badge}
              </span>
            </div>
            <p className="text-sm text-gray-600 mb-1">{meeting.time}</p>
            <p className="flex items-center text-xs text-gray-500 space-x-2">
        ...
        <button className="w-full text-center text-sm text-gray-600 hover:text-gray-800 py-2 border border-gray-200 rounded-xl">
          View All Meetings
        </button>
      </div>
    </div>
